...
if [ ! $USER ]; then
  	echo "You must specify the userName used when starting vscodium1_64_2forcora2TempSetup"
else
cd vscodium1_64_2forcora2
...
${CONTAINERRUNTIME} run --rm -ti --privileged --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
 ...
 --name vscodium1_64_2forcora2TempSetup\
 vscodium1_64_2forcora2
 cd ../
fi
