...
INSTALLVERSION=vscodium1_64_2forcora1
...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building vscodium1_64_2forcora1
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building vscodium1_64_2forcora1, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./vscodiumForCora ]; then
	echo you must specify the dockergroupid to be used when building vscodium1_64_2forcora1, use: getent group docker 
else
	if [ ! -d ./vscodium1_64_2forcora1 ]; then
...
#	./vscodiumForCora/startVSCodiumForCora.sh $USER
	./vscodium1_64_2forcora1/startVSCodiumForCoraTempSetup.sh $USER $VSCODIUMBRANCH $RECOMMENDEDSETUP
