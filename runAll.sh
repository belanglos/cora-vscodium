...
INSTALLVERSION=vscodium1_67_2forcora2
...
if [ ! $USER ]; then
  	echo you must specify the userName to be used when building vscodium1_67_2forcora2
elif [ ! $USERID ]; then
	echo you must specify the userid to be used when building vscodium1_67_2forcora2, use: id -u youruserid 
elif [ ! $DOCKERGROUPID ] && [ ! -d ./vscodiumForCora ]; then
	echo you must specify the dockergroupid to be used when building vscodium1_67_2forcora2, use: getent group docker 
else
	if [ ! -d ./vscodium1_67_2forcora2 ]; then
...
#	./vscodiumForCora/startVSCodiumForCora.sh $USER
	./vscodium1_67_2forcora2/startVSCodiumForCoraTempSetup.sh $USER $VSCODIUMBRANCH $RECOMMENDEDSETUP
