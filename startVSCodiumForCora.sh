...
if [ ! $USER ]; then
  	echo "You must specify the userName used when starting vscodium1_67_2forcora1"
else
	#${CONTAINERRUNTIME} run --rm -ti --privileged --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
cd vscodium1_67_2forcora1
...
${CONTAINERRUNTIME} run --rm -ti --privileged  --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
 ...
 -p 33001:3000\
 --name vscodium1_67_2forcora1\
 vscodium1_67_2forcora1 $2
 cd ../
fi
