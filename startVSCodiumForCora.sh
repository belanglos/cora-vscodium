...
if [ ! $USER ]; then
  	echo "You must specify the userName used when starting vscodium1_67_2forcora2"
else
	#${CONTAINERRUNTIME} run --rm -ti --privileged --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
cd vscodium1_67_2forcora2
...
${CONTAINERRUNTIME} run --rm -ti --privileged  --ipc=host --env="QT_X11_NO_MITSHM=1"  -e DISPLAY=$DISPLAY \
 ...
 -p 33001:3000\
 --name vscodium1_67_2forcora2\
 vscodium1_67_2forcora2 $2
 cd ../
fi
